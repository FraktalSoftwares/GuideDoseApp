...
class GuideDoseSupabaseUser extends BaseAuthUser {
  GuideDoseSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> guideDoseSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = GuideDoseSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
